...
const STATUS_COLORS = {
  REVENUE: { bg: "rgba(16, 185, 129, 0.1)", text: "var(--success)", icon: <ArrowUpRight size={16} /> },
  EXPENSE: { bg: "rgba(239, 68, 68, 0.1)", text: "var(--danger)", icon: <ArrowDownRight size={16} /> },
  WITHDRAWAL: { bg: "rgba(245, 158, 11, 0.1)", text: "var(--warning)", icon: <ArrowDownRight size={16} /> },
  TRANSFER: { bg: "#eff6ff", text: "var(--primary-color)", icon: <RefreshCcw size={16} /> },
};
...
function KpiCard({ title, value, type, icon }) {
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 12, padding: "20px", display: "flex", flexDirection: "column", gap: 8 }}>
  ...
  return (
    <div style={{ padding: "24px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: "bold", color: "var(--text-main)", margin: 0 }}>💰 Mes Bénéfices</h1>
          <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4, margin: 0 }}>Gérez vos revenus, dépenses et retraits personnels.</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <select value={period} onChange={(e) => setPeriod(e.target.value)}
            style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border-color)", background: "var(--bg-card)", outline: "none", fontSize: 13 }}>
          ...
          <button onClick={fetchData} style={{ padding: "8px", borderRadius: 8, border: "1px solid var(--border-color)", background: "var(--bg-card)", cursor: "pointer" }}>
            <RefreshCcw size={16} color="#64748b" />
          </button>
          <button onClick={() => openModal()} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, border: "none", background: "var(--primary-color)", color: "var(--bg-card)", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
      ...
      {r?.kpis?.availableBalance < 0 && (
        <div style={{ background: "rgba(239, 68, 68, 0.1)", borderLeft: "4px solid #ef4444", padding: "12px 16px", borderRadius: 8, marginBottom: 24, display: "flex", alignItems: "center", gap: 12, color: "#991b1b" }}>
          <Info size={20} />
          <div>
      ...
      {loading && !r ? (
        <div style={{ textAlign: "center", padding: 60, color: "var(--text-muted)" }}>⏳ Chargement des données...</div>
      ) : r ? (
        <>
          {/* KPIS */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 24 }}>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 12, padding: "20px" }}>
              <div style={{ fontWeight: 600, color: "var(--text-main)", marginBottom: 16 }}>Évolution (Derniers 6 mois)</div>
              <div style={{ display: "flex", gap: 12, marginBottom: 12, fontSize: 11, color: "var(--text-muted)" }}>
                <span style={{display: "flex", alignItems: "center", gap: 4}}><div style={{width: 10, height: 10, background: "var(--success)", borderRadius: 2}}/> Revenus</span>
                <span style={{display: "flex", alignItems: "center", gap: 4}}><div style={{width: 10, height: 10, background: "var(--danger)", borderRadius: 2}}/> Dépenses</span>
                <span style={{display: "flex", alignItems: "center", gap: 4}}><div style={{width: 10, height: 10, background: "var(--warning)", borderRadius: 2}}/> Retraits</span>
              </div>
              <MiniBarChart data={r.monthlyTrend} currency={currency} />
            </div>

            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 12, padding: "20px" }}>
              <div style={{ fontWeight: 600, color: "var(--text-main)", marginBottom: 16 }}>Résumé Mensuel (Derniers 6 mois)</div>
                  ...
                  <tbody>
                    {r.monthlyTrend.slice().reverse().map(m => (
                      <tr key={m.month} style={{ borderBottom: "1px solid var(--border-color)" }}>
                        ...
                        <td style={{ padding: "8px 0", textAlign: "right", color: "var(--success)" }}>{formatCurrency(m.revenue)}</td>
                        <td style={{ padding: "8px 0", textAlign: "right", color: "var(--danger)" }}>{formatCurrency(m.expenses)}</td>
                        <td style={{ padding: "8px 0", textAlign: "right", fontWeight: 600 }}>{formatCurrency(m.profit)}</td>
                        <td style={{ padding: "8px 0", textAlign: "right", color: "var(--warning)" }}>{formatCurrency(m.withdrawals)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border-color)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div style={{ fontWeight: 600, color: "var(--text-main)", fontSize: 16 }}>Transactions</div>
              
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", background: "var(--bg-app)", border: "1px solid var(--border-color)", borderRadius: 6, padding: "4px 8px" }}>
                ...
                <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
                  style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid var(--border-color)", fontSize: 12, outline: "none", background: "var(--bg-app)" }}>
                ...
                <button onClick={exportCSV} title="Exporter CSV"
                  style={{ padding: "6px 8px", borderRadius: 6, border: "1px solid var(--border-color)", background: "var(--bg-app)", cursor: "pointer", display: "flex", alignItems: "center" }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "var(--bg-app)", color: "var(--text-muted)", borderBottom: "1px solid var(--border-color)" }}>
                  ...
                  {filteredTransactions.length === 0 ? (
                    <tr>
                      <td colSpan={7} style={{ padding: "40px", textAlign: "center", color: "var(--text-muted)" }}>
                        ...
                        <button onClick={() => openModal()} style={{ background: "transparent", border: "1px dashed var(--border-color)", padding: "8px 16px", borderRadius: 8, color: "var(--primary-color)", cursor: "pointer", fontWeight: 500 }}>
                    ...
                    filteredTransactions.map(tx => (
                      <tr key={tx.id} style={{ borderBottom: "1px solid var(--border-color)" }}>
                        <td style={{ padding: "12px 20px", color: "var(--text-main)" }}>{tx.date.split("T")[0]}</td>
                        ...
                        <td style={{ padding: "12px 20px", color: "var(--text-main)", fontWeight: 500 }}>{tx.category}</td>
                        <td style={{ padding: "12px 20px", color: "var(--text-muted)", maxWidth: 200, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tx.description || "-"}</td>
                        ...
                        <td style={{ padding: "12px 20px", textAlign: "center" }}>
                          {tx.isEditable ? (
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                              <button onClick={() => openModal(tx)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--primary-color)" }}><Edit2 size={16} /></button>
                              <button onClick={() => handleDelete(tx.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--danger)" }}><Trash2 size={16} /></button>
                            </div>
                          ) : (
                            <span style={{ fontSize: 11, color: "var(--text-muted)", background: "var(--bg-hover)", padding: "2px 6px", borderRadius: 4 }}>Auto</span>
                          )}
                        </td>
                      </tr>
                    ))
      ...
      {showModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "var(--bg-card)", width: "100%", maxWidth: 500, borderRadius: 16, overflow: "hidden", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg-app)" }}>
              <h2 style={{ margin: 0, fontSize: 18, color: "var(--text-main)" }}>{editingTx ? "Modifier la transaction" : "Ajouter une transaction"}</h2>
              <button onClick={() => setShowModal(false)} style={{ background: "none", border: "none", fontSize: 24, cursor: "pointer", color: "var(--text-muted)" }}>&times;</button>
            </div>
            
            <form onSubmit={handleSave} style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Type</label>
                  <select value={form.type} onChange={e => {
                      const newType = e.target.value;
                      setForm({...form, type: newType, category: CATEGORIES[newType][0]});
                    }}
                    style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid var(--border-color)", outline: "none", background: "var(--bg-card)" }}>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Date</label>
                  <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} required
                    style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid var(--border-color)", outline: "none", boxSizing: "border-box" }} />
                </div>
              </div>

              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Montant (DA)</label>
                <input type="number" step="0.01" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} required
                  style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid var(--border-color)", outline: "none", boxSizing: "border-box", fontSize: 16, fontWeight: "bold" }} placeholder="0.00" />
              </div>

              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Catégorie</label>
              ...
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Description / Note</label>
                <input type="text" value={form.description} onChange={e => setForm({...form, description: e.target.value})}
                  style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid var(--border-color)", outline: "none", boxSizing: "border-box" }} placeholder="Détails de la transaction..." />
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Méthode de paiement</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-main)", marginBottom: 6 }}>Source / Client (Optionnel)</label>
                  <input type="text" value={form.client} onChange={e => setForm({...form, client: e.target.value})}
                    style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid var(--border-color)", outline: "none", boxSizing: "border-box" }} placeholder="Nom du client..." />
                </div>
              </div>

              <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 16 }}>
                <button type="button" onClick={() => setShowModal(false)}
                  style={{ padding: "10px 20px", borderRadius: 8, border: "1px solid var(--border-color)", background: "var(--bg-card)", fontWeight: 600, color: "var(--text-main)", cursor: "pointer" }}>
                  Annuler
                </button>
                <button type="submit" disabled={submitting}
                  style={{ padding: "10px 24px", borderRadius: 8, border: "none", background: "var(--primary-color)", fontWeight: 600, color: "var(--bg-card)", cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.7 : 1 }}>
