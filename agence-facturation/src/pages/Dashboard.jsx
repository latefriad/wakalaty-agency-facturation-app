...
const STATUS = {
  ...
  SENT: { color: "var(--primary-color)", bg: "#dbeafe" },
  EN_ATTENTE: { color: "var(--warning)", bg: "#fef3c7" },
  PAYEE: { color: "var(--success)", bg: "#d1fae5" },
  ANNULEE: { color: "var(--danger)", bg: "#fee2e2" },
};
...
export default function Dashboard() {
  ...
  if (loading && !dash) return <div style={{ textAlign: "center", padding: 80, color: "var(--text-muted)", fontSize: 16 }}>⏳ {t("common.loading")}</div>;

  if (error)
    return (
      <div style={{ textAlign: "center", padding: 80, direction: "inherit", fontFamily: "'Segoe UI',Tahoma,sans-serif" }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>⚠️</div>
        <h2 style={{ color: "var(--danger)", marginBottom: 8, fontSize: 18 }}>{t("dash.loadError")}</h2>
        <p style={{ color: "var(--text-muted)", marginBottom: 16, fontSize: 14 }}>{t("dash.checkConnection")}</p>
        <button onClick={() => { setError(null); window.location.reload(); }} style={{ padding: "10px 24px", borderRadius: 10, background: "var(--primary-color)", color: "var(--bg-card)", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>{t("common.retry")}</button>
      </div>
    );
    ...
    <div style={{ direction: "inherit", fontFamily: "'Segoe UI', Tahoma, sans-serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 12, marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: "var(--text-muted)", margin: "4px 0 0", fontSize: 14 }}>{t("dash.subtitle")}</p>
        </div>
        ...
        <div className="no-print" style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button onClick={() => window.print()}
            style={{ padding: "7px 12px", borderRadius: 8, border: "1px solid var(--border-color)", background: "var(--bg-card)", color: "var(--text-main)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
            🖨️ PDF
          </button>
          <span style={{ width: 8 }} />
          {[
            ...
            <button key={p.key} onClick={() => setPeriod(p.key)}
              style={{ padding: "7px 12px", borderRadius: 8, border: "1px solid", borderColor: period === p.key ? "#3b82f6" : "var(--border-color)", background: period === p.key ? "#3b82f6" : "var(--bg-card)", color: period === p.key ? "#fff" : "var(--text-main)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
              {p.label}
            </button>
          ))}
          {period === "custom" && (
      ...
      <div style={{ background: "linear-gradient(135deg, #0f172a, #1e3a5f)", borderRadius: 16, padding: "24px 28px", marginBottom: 20, color: "var(--bg-card)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px,1fr))", gap: 16, marginBottom: 28 }}>
        {[
          { label: t("dash.received"), sub: t("dash.receivedHint"), value: `${fmt(dash?.cash?.received)} ${cur}`, icon: "💰", color: "var(--success)", bg: "#d1fae5" },
          ...
          { label: t("dash.netProfit"), sub: dash?.profit?.margin != null ? `${t("dash.margin")} ${(dash.profit.margin * 100).toFixed(1)}%` : t("dash.noCashYet"), value: `${fmt(dash?.profit?.net)} ${cur}`, icon: "🏆", color: (dash?.profit?.net ?? 0) >= 0 ? "#10b981" : "var(--danger)", bg: (dash?.profit?.net ?? 0) >= 0 ? "#d1fae5" : "#fee2e2" },
          { label: t("dash.billed"), sub: t("dash.billedHint"), value: `${fmt(dash?.billed?.total)} ${cur}`, icon: "🧾", color: "var(--primary-color)", bg: "#dbeafe" },
          { label: t("dash.outstandingCard"), sub: `${dash?.outstanding?.count ?? 0} ${t("dash.invoicesShort")}`, value: `${fmt(dash?.outstanding?.total)} ${cur}`, icon: "⏳", color: "var(--warning)", bg: "#fef3c7" },
          { label: t("dash.overdue", { count: dash?.outstanding?.overdueCount ?? 0 }), sub: t("dash.overdueHint"), value: `${fmt(dash?.outstanding?.overdue)} ${cur}`, icon: "⏰", color: "var(--danger)", bg: "#fee2e2" },
          ...
          <div key={card.label} style={{ background: "var(--bg-card)", borderRadius: 14, padding: "20px 22px", border: "1px solid var(--border-color)", display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {card.sub && <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{card.sub}</div>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px,1fr))", gap: 16, marginBottom: 28 }}>
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 16 }}>{t("dash.agingHint")}</div>
          {[
            { label: t("dash.agingNotDue"), value: dash?.aging?.notDue ?? 0, color: "var(--success)" },
            { label: "1–30 " + t("dash.days"), value: dash?.aging?.days1to30 ?? 0, color: "var(--warning)" },
            { label: "31–60 " + t("dash.days"), value: dash?.aging?.days31to60 ?? 0, color: "#f97316" },
            { label: "60+ " + t("dash.days"), value: dash?.aging?.days60plus ?? 0, color: "var(--danger)" },
          ].map((b) => {
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 14 }}>{t("dash.worstOverdueHint")}</div>
          {(dash?.worstOverdue ?? []).length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--text-muted)", padding: 20 }}>✅ {t("dash.noOverdue")}</div>
          ) : (
            (dash?.worstOverdue ?? []).map((w) => (
              <div key={w.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid var(--border-color)" }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--primary-color)" }}>{w.number}</div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{w.clientName}</div>
                </div>
                <div style={{ textAlign: "end", flexShrink: 0 }}>
                  <div style={{ fontWeight: 700, color: "var(--danger)", fontSize: 14 }}>{fmt(w.remaining)} {cur}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px,1fr))", gap: 16, marginBottom: 28 }}>
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 16 }}>{t("dash.receivedHint")}</div>
          {cashSeries.length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--text-muted)", padding: 20 }}>{t("dash.noDataYet")}</div>
          ) : (
            <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 140 }}>
              {cashSeries.map((m) => (
                <div key={m.month} title={`${m.month}: ${fmt(m.received)} ${cur}`} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  <div style={{ width: "100%", borderRadius: "6px 6px 0 0", background: "var(--success)", height: `${Math.max(4, (m.received / maxMonthly) * 110)}px` }} />
                  <span style={{ fontSize: 10, color: "var(--text-muted)" }}>{m.month.slice(5)}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          {(dash?.statusBreakdown ?? []).length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--text-muted)", padding: 20 }}>{t("dash.noInvoicesYet")}</div>
          ) : (
            (dash?.statusBreakdown ?? []).map((row) => {
              ...
              return (
                <div key={row.status} style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 13 }}>
                    <span style={{ color: "var(--text-main)" }}>{t(`status.${row.status}`)}</span>
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          {(dash?.expenses?.byCategory ?? []).length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--text-muted)", padding: 20 }}>{t("dash.noExpensesYet")}</div>
          ) : (
            (dash?.expenses?.byCategory ?? []).map((row) => {
              ...
              return (
                <div key={row.category} style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 13 }}>
                    <span style={{ color: "var(--text-main)" }}>{t(`exp.cat.${row.category}`)}</span>
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 14 }}>{t("dash.topClientsHint")}</div>
          {(dash?.topClients ?? []).length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--text-muted)", padding: 20 }}>{t("dash.noRevenueYet")}</div>
          ) : (
            (dash?.topClients ?? []).map((c, i) => (
              <div key={c.clientId} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid var(--border-color)" }}>
                <span style={{ fontSize: 14 }}>{["🥇", "🥈", "🥉", "4.", "5."][i]} {c.name}</span>
                <span style={{ fontWeight: 700, color: "var(--success)", fontSize: 14 }}>{fmt(c.received)} {cur}</span>
              </div>
            ))
          )}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px,1fr))", gap: 16, marginBottom: 28 }}>
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 16 }}>{t("dash.budgetHint")}</div>
          {!dash?.budget?.defined ? (
            ...
            <>
              ...
              <div style={{ marginBottom: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 13 }}>
                  <span style={{ color: "var(--text-main)", fontWeight: 600 }}>💰 {t("dash.revenueTarget")}</span>
                  <span style={{ fontWeight: 700, color: dash.budget.revenueActual >= dash.budget.revenueTarget ? "#10b981" : "var(--text-main)" }}>
                    {fmt(dash.budget.revenueActual)} / {fmt(dash.budget.revenueTarget)} {cur}
                  </span>
                </div>
                <div style={{ background: "var(--bg-hover)", borderRadius: 99, height: 12 }}>
                  <div style={{ width: `${Math.min(100, Math.round((dash.budget.revenueActual / Math.max(1, dash.budget.revenueTarget)) * 100))}%`, height: 12, borderRadius: 99, background: "var(--success)", transition: "width 0.5s ease" }} />
                </div>
              </div>
                ...
                return (
                  <div key={row.category} style={{ marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, fontSize: 12 }}>
                      <span style={{ color: "var(--text-main)" }}>{t(`exp.cat.${row.category}`)}{over && " ⚠️"}</span>
                      <span style={{ color: over ? "#dc2626" : "var(--text-muted)", fontWeight: 600 }}>
                        {fmt(row.actual)} / {row.budget > 0 ? fmt(row.budget) : "—"} {cur}
                      </span>
                    </div>
                    <div style={{ background: "var(--bg-hover)", borderRadius: 99, height: 8 }}>
                      <div style={{ width: `${pct}%`, height: 8, borderRadius: 99, background: over ? "#dc2626" : "var(--primary-color)", transition: "width 0.5s ease" }} />
                    </div>
                  </div>
                );
              })}
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 14 }}>{t("dash.forecastHint")}</div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "8px 0", borderBottom: "2px solid var(--border-color)", marginBottom: 4 }}>
            <span style={{ color: "var(--text-main)", fontWeight: 600 }}>{t("dash.forecastStart")}</span>
            <span style={{ fontWeight: 700 }}>{fmt(dash?.forecast?.startingBalance)} {cur}</span>
          </div>
          {(dash?.forecast?.months ?? []).map((m) => (
            <div key={m.month} style={{ padding: "10px 0", borderBottom: "1px solid var(--border-color)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
                <span style={{ fontWeight: 600, color: "var(--text-main)" }}>📅 {m.month}</span>
                <span style={{ fontWeight: 800, color: m.projectedBalance >= 0 ? "#10b981" : "var(--danger)" }}>
                  {m.projectedBalance < 0 && "⚠️ "}{fmt(m.projectedBalance)} {cur}
                </span>
              </div>
              <div style={{ display: "flex", gap: 14, fontSize: 11, color: "var(--text-muted)", marginTop: 3 }}>
                <span style={{ color: "var(--success)" }}>+{fmt(m.expectedIn)} {t("dash.forecastIn")}</span>
      ...
      <div style={{ background: "var(--bg-card)", borderRadius: 14, padding: 24, border: "1px solid var(--border-color)", marginBottom: 28 }}>
        <h3 style={{ margin: "0 0 6px", fontSize: 15, fontWeight: 600 }}>{t("dash.clientProfit")}</h3>
        <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 14 }}>{t("dash.clientProfitHint")}</div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {(dash?.clientProfit ?? []).map((c) => (
                  <tr key={c.clientId} style={{ borderTop: "1px solid var(--border-color)" }}>
                    <td style={{ padding: "9px 10px", fontWeight: 600 }}>{c.name}</td>
                    <td style={{ padding: "9px 10px", textAlign: "end", color: "var(--success)", fontWeight: 600 }}>{fmt(c.received)}</td>
                    <td style={{ padding: "9px 10px", textAlign: "end", color: "var(--warning)" }}>{fmt(c.outstanding)}</td>
                    <td style={{ padding: "9px 10px", textAlign: "end", color: "var(--text-muted)" }}>{fmt(c.directCosts)}</td>
                    <td style={{ padding: "9px 10px", textAlign: "end", fontWeight: 700, color: c.profit >= 0 ? "#10b981" : "var(--danger)" }}>
                      {fmt(c.profit)} {cur}
                    </td>
                  </tr>
                ))}
        ...
        {(dash?.recentInvoices ?? []).length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {(dash?.recentInvoices ?? []).map((inv) => {
              ...
              return (
                <div key={inv.id} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", padding: "12px", borderBottom: "1px solid var(--border-color)", fontSize: 14, alignItems: "center" }}>
                  <span style={{ fontWeight: 600, color: "var(--primary-color)" }}>{inv.number || t("inv.draftLabel")}</span>
                  <span style={{ color: "var(--text-main)" }}>{inv.client?.name || t("dash.client")}</span>
